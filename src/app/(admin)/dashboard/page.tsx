...
export default function DashboardPage() {
  ...
  const kpis = [
    ...
    { name: 'Transcripts', value: stats?.totalTranscripts, icon: FileText, color: 'text-indigo-600', bg: 'bg-indigo-100' },
    { name: 'Pain Points', value: stats?.totalPainPoints, icon: AlertTriangle, color: 'text-amber-600', bg: 'bg-amber-100' },
    { name: 'Fuentes Extraídas', value: stats?.totalSources, icon: Brain, color: 'text-purple-600', bg: 'bg-purple-100' },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Main Stats Card */}
        <div className="lg:col-span-2 glass-card p-8 border-slate-800/50 overflow-hidden relative">
          ...
          {/* Decorative SVG Line Chart Background */}
          <div className="absolute bottom-0 left-0 right-0 h-32 opacity-10 pointer-events-none">
            <svg viewBox="0 0 400 100" className="w-full h-full preserve-3d">
               <path d="M0,80 Q50,20 100,50 T200,30 T300,70 T400,40" fill="none" stroke="currentColor" strokeWidth="2" className="text-indigo-500" />
            </svg>
          </div>
        </div>
            ...
            <div className="space-y-6 flex-1">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="font-bold text-white text-sm">Gemma 3 (OpenRouter)</p>
                </div>
              </div>
            </div>
